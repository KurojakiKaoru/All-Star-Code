...
// array variable.
heroCreator.poplulateGrid(gridDiv, heroes);
...
function removeIds() {
    document.getElementsByTagName("allHeroContainers")[0].removeAttribute("id");
}
...
function highlightHero() {
    removeIds();
    currentTarget.setAttribute(highlight);
}

// 10) Loop through each item in the "allHeroContainers" you just made 
// and set the on click event listener as "highlightHero"
for(let hero_loop in allHeroContainers) {
    hero_loop.addEventListener('click', highlightHero, false);
}
...
function click(){
    ...
    if(titleInput.value == "" || typeInput.value == "" || difficultyInput.value == "" || sayingInput.value == ""){
        alert("Please fill in all fields");
    }else{
        let title = titleInput.value;
        let type = typeInput.value;
        let difficulty = difficultyInput.value;
        let saying = sayingInput.value;
    }
